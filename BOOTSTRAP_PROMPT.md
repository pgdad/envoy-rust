...
9. **`STATE.md` hygiene — relocate superseded narrative every session (per ADR-0035).** `STATE.md` accumulates a `_Historical (…superseded…)_` block under each top section (`## Active phase` / `## Next expected skill` / `## Last commit` / `## Last updated`) plus a per-phase `### Phase-NN …` Notes subsection on every state commit, so it regrows toward the un-readable size ADR-0035 fixed. **As a standing part of every state-mutating session** (do it alongside the STATE.md update, before the commit): move the now-superseded narrative **verbatim, byte-for-byte** out of `STATE.md` into `docs/envoy-rust/STATE_HISTORY.md` and leave only the breadcrumb pointer. Concretely — (a) under each of the four top sections, keep ONLY the current entry and the `> Historical … preserved verbatim in STATE_HISTORY.md per ADR-0035` breadcrumb; relocate every `_Historical_` sub-entry that accumulated. (b) Under `## Notes`, keep `### Doctrine reminders` + the **active phase's** subsections + the breadcrumbs; relocate the `### Phase-NN …` subsections of every now-closed phase. (c) In `STATE_HISTORY.md`, **prepend** the relocated top-section blocks immediately under their matching `## Relocated from STATE.md …` header (the archive is newest-first, and STATE.md's `_Historical_` block is already newest-first, so it dovetails), and **append** a new `## Relocated from STATE.md \`## Notes\` — closed-phase subsections (phases X–Y)` section at EOF for the relocated Notes. (d) **Verify byte-preservation** before committing: the multiset of relocated lines must exactly equal `(old STATE.md) − (new STATE.md)`, and every relocated line must appear in `STATE_HISTORY.md`. Never summarize or delete a narrative (D-3.5); relocation is the only permitted shrink. A green target is `STATE.md` back near ~120 lines.
