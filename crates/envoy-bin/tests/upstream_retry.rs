...
async fn retry_success_and_limit_exceeded_paths() {
    ...
    // -------------------------------------------------------------------------
    let (status_a, headers_a, body_a) = http1_oneshot(hcm_addr, "/retry-success", "backend").await;
    assert_eq!(
        status_a, 200,
    ...
    // -------------------------------------------------------------------------
    let (status_b, headers_b, body_b) =
        http1_oneshot(hcm_addr, "/retry-exhausted", "backend").await;
    assert_eq!(
        status_b, 503,
