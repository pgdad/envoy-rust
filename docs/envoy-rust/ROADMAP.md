...
| 05 | HTTP/2 downstream + upstream (low-level framer, own conn mgr) | 04 | in-progress | 05.1, 05.2, 05.3 | HTTP/2 fixture green; `h2spec` above threshold |
| 05.1 | Fixture-hardening preamble: `ClusterType::StrictDns` + 5-fixture coordinated edit + closes phase-04.3 REVIEW C-1 + closes phase-02.1 REVIEW I3 | 04 | planned | — | no new fixture; fixtures 0003/0004/0005/0006/0008 restored to Docker-gated green; envoy-config schema gains `STRICT_DNS` cluster type; ADR-0023 lands |
| 05.2 | `envoy-http2` foundation + downstream H2C HCM + fixture 0009 + `h2spec` ≥95% gate | 05.1 | planned | — | fixture 0009-http2-direct-response green; envoy-http2 crate (sole dep on `h2`) + listener-side `Http2ProtocolOptions` + HCM-on-H2 dispatch + harness `Driver::Http2` + tests/conformance/h2spec/ runner at ≥95% pass |
| 05.3 | Upstream H2C origination + router H2-arm + http2-echo-server helper + fixture 0010 | 05.2 | planned | — | fixture 0010-http2-router-upstream green; envoy-http2::Client lands; router H2-arm dispatches H1-or-H2 by cluster.upstream_protocol; parent phase 05 flips done |
