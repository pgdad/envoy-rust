...
## State-5 code review (`superpowers:requesting-code-review`) — `REVIEW.md` APPROVED

State-5 `superpowers:requesting-code-review` reviewed the `25.1` state-3 implementation (git range `7257bc619` [parent of Task-1 `20ce64010`] … `0c8b66512` [end of state-3]; production change isolated to `crates/envoy-http1/src/hcm.rs`, +379, plus the BEHAVIOR_CONTRACT note). Two independent read-only review subagents were dispatched (one for correctness + buffer-advance arithmetic + regression-equivalence + per-attempt replay + scope fidelity; one for test-validity + flakiness + code quality), their findings synthesized and adjudicated into `REVIEW.md`.

**Verdict: APPROVED — 0 Critical, 0 blocking-Important, Minor-only.** Both reviewers independently returned "Ready to merge: Yes."

- **Verified correct:** the buffer-advance arithmetic (`hcm.rs:639-667`, byte-for-byte equivalent to the removed drain — no double-advance / off-by-one / leftover-bytes); the `body_len == 0` no-op regression-equivalence invariant; the chunked-501 preservation (`:694`; a real chunked request carries no Content-Length → `body_len==0` → no-op); per-attempt replay safety (`:359`, `run_attempt` takes `&Request`, non-consuming clone); the verbatim error/timeout dispositions (the relocation did not cross the response-write boundary, so mid-body-timeout → graceful close was pre-existing); exact scope fidelity (no `decode_data`/`BufferFilter`/new fixture/new crate — all deferred to `25.2`). The 5 tests assert REAL forwarded bytes — the forwarding (`h1_forwards_request_body_upstream`) and retry-replay (`h1_retried_post_replays_body`, exactly-two invariant) tests genuinely fail on the pre-change empty-body code; the keep-alive bleed test is truly single-connection; capture-ordering races are provably avoided (record-before-respond + await-EOF).
- **The one finding the correctness reviewer flagged "Important" was adjudicated to non-blocking Minor (M25.1-1):** eager `BytesMut::with_capacity(body_len)` (`hcm.rs:641`) reserves the untrusted / uncapped client Content-Length. Downgraded because it is PLAN-specified (Task 1 Step 4), was consciously deferred in state-3 (Task 1 Minor (a) above), is low practical severity on the default Linux/overcommit target (only received bytes fault in), and `max_request_bytes` enforcement is `25.2`'s locked deliverable. Architectural note carried to the `25.2` planner: the BufferFilter's post-read `body.len() > max` check (in `decode_headers`, after `25.1` has already buffered the whole body per SPEC §5.3) does NOT by itself bound the read/allocation — `25.2` should bound the read in `hcm.rs`.
- **Other Minors (all non-blocking, recorded in `REVIEW.md`):** M25.1-2 cross-TCP-segment body reassembly is not exercised by a forwarding test (mitigated — the loop is a verbatim relocation of the previously-tested drain); M25.1-3 loose `contains` assertions in the keep-alive test (not vacuous given the request-line + response-count checks); M25.1-4 `drive_keep_alive` status-line split heuristic; M25.1-5 ~10 lines helper read-loop duplication; M25.1-6 the 200 ms recording-helper idle timeout; M25.1-7 a stale `:599` comment. The previously-deferred PROGRESS Task 1 Minor (b) (bogus-Content-Length-on-chunked buffering) was traced and **CONFIRMED a non-concern** (behavior-preserving — the 501 path never forwards `req.body`; both old and new consume exactly `body_len` bytes). M25.1-1 + M25.1-2 are carried into the `25.2` PLAN-write.

Per `BOOTSTRAP_PROMPT.md` §5.2 there is NO Critical/Important blocker, so the phase does NOT re-enter state 3; it advances to the state-6 deterministic close-out. NO new ADR opened (a code-review session records findings, not decisions); DECISIONS.md ledger head stays ADR-0064. Per §5.1 (one state per session) this session advances `STATE.md` → state-5-complete / state-6-next (next: the state-6 deterministic close-out) and EXITS. It does NOT begin the state-6 close-out.
