...
- **PV-4 (MEASURED both paths):** upstream ROUTE header matching AND access-log `header_filter` BOTH drop absent+`invert_match` (route: `GET /` with no header + inverted matcher → 202 fallback, not the 201 inverted route; access-log: absent+invert → not logged). The in-tree shared engine's `mode_result ^ invert_match` (`matcher.rs:51`) yields absent+invert = KEEP → a **latent SHARED bug on BOTH the route and access-log paths** (not access-log-specific). Scoped OUT of phase 72; documented; owner is new carry-forward **CF-72-1**.
  - **[CORRECTED at §5.2 state-3 — ADR-0151, D-3.5 strike-don't-delete]:** the divergence is **MODE-DEPENDENT**, not the blanket "absent+invert" above. The state-2 measurement above used a VALUE matcher (`string_match/exact`), which DOES diverge. But the state-5 LIVE-PROBE MEASURED that `present_match`+invert+absent is **PARITY** (envoy-rust AND upstream BOTH KEEP), while only VALUE-based matchers (`exact`/`prefix`/`suffix`/`safe_regex`/`range`/`string_match`) + invert + absent diverge (envoy-rust KEEP vs upstream DROP). **CF-72-1 is re-scoped to the value-matcher case** — a future fixer MUST preserve the `present_match` KEEP. See `BEHAVIOR_CONTRACT.md` §C + the mode-scoped pins in `matcher.rs`.
