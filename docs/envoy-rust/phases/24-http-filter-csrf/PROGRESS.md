...
- **AUTHORITATIVE Linux CI anchor (ADR-0049 Provenance):** run [`27457698815`](https://github.com/pgdad/envoy-rust/actions/runs/27457698815) at HEAD `9b0e7b925`, conclusion **success** — both jobs green: `build + test + lint` (`cargo test --workspace` → the differential harness over Docker on Linux, incl. `test http_filter_csrf_fixture ... ok` [fixture `0032`] + `test http_filter_csrf_in_process_backstop ... ok` + the full csrf validator/typed_per_filter_config suite) and `fuzz (parse_bootstrap + jwt_parse, 30s each)`. This supersedes the phase-23 anchor `27317400787` (code-HEAD `ff3721fd1`) as the project's differential evidence of record.
