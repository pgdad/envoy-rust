...
**directory:** `docs/envoy-rust/phases/05.2-http2-downstream/` — contains `SPEC.md` only (committed at the parent-05 state-2 commit `f1804a7`; PLAN.md / PROGRESS.md / REVIEW.md land in subsequent state transitions). Sibling directories `05.1-fixture-hardening/` (closed at `1d05cd0`; contains `SPEC.md` + `PLAN.md` + `PROGRESS.md` + `REVIEW.md`), `05.4-fixture-hardening-followup/` (closed at this commit; contains `SPEC.md` + `PLAN.md` + `PROGRESS.md` + `REVIEW.md`), and `05.3-http2-upstream/` (SPEC.md only) exist alongside. Parent directory `docs/envoy-rust/phases/05-http2/` continues to hold the parent SPEC at `SPEC.md` (committed at parent-05 state-1 commit `cd1a70e`; 490 lines, unedited per D-3.4 / D-3.5 — the parent SPEC remains the historical artifact projecting the original 3-way split, superseded for execution by the four sub-phase SPECs).
**status:** lifecycle **state 2 (SPEC.md exists, PLAN.md does not)** as of this commit. ROADMAP row `05` remains `status: in-progress` (will flip to `done` only after ALL parent-05 sub-phases close, per ROADMAP-schema invariant). ROADMAP row `05.4` flips `in-progress` → `done` at this commit. ROADMAP row `05.2` remains `status: planned`; **the active-phase pointer in STATE.md is the operative gate during execution** — 05.2 enters in-progress only at its own state-2 PLAN-writing session per `BOOTSTRAP_PROMPT.md` §5.1. ROADMAP row `05.3` remains `status: planned`. 05.2's `depends-on: 05.1` ROADMAP-row content stays unedited per the ROADMAP append-only rule (only `status` and `sub-phases` columns mutate); the strict execution ordering after 05.4 closes is 05.2 → 05.3, with 05.3's PLAN-writing entry waiting for 05.2's state-6 close-out.

**Phase 05.4 (`05.4-fixture-hardening-followup`) is done as of this commit (the phase-05.4 state-6 close-out).** The 6 root-cause fixes substantively closing phase-04.3 REVIEW C-1 landed in 7 substantive tasks across 9 commits between base `1d05cd0` and head `a8c2364`, plus this state-6 close-out commit. Tasks: Task 1 (`3d0af4a`) added `Cluster.dns_lookup_family` field + `DnsLookupFamily` enum to envoy-config + ADR-0024; Task 2 (`2bf4ba1`) flipped 5 fixtures' envoy.yaml to set `dns_lookup_family: V4_ONLY`; Task 3 (`f1dbe12`) introduced the `Listener.listener_filters` parse-and-ignore field + fixture 0006 `tls_inspector` block + ADR-0026; Task 4 (`b72f1cb`) flipped the 3 echo-server helpers' production binds from `127.0.0.1` to `0.0.0.0`; Task 5 (`01edie3`) suppressed the synthetic `content-length: 0` on empty-body requests in `envoy-http1::Client` + fixture 0008 expectations.yaml drop + ADR-0025; Task 6 (`06c706d`) bumped STRICT_DNS settle time 500ms → 2000ms for `host_gateway = true` fixtures; Task 7 (`a8c2364`) materialized the state-4 phase-done gate evidence (CI run `25276504502` shows all 8 Docker-gated fixtures green simultaneously). Phase 05.4 `REVIEW.md` (landed at `a88a986`) verdict is **Approved with M-track follow-ups** — see "Phase-05.4 rollovers" below for the carryforward disposition. **The phase-04.3 REVIEW C-1 carryforward chain ends here** (originating at phase-02.2 ADR-0015 landing `435c6fa`, latent across phases 02.2 → 03.1 → 03.2 → 04.1 → 04.2 → 04.3, partially closed at 05.1 state-6 `1d05cd0`, substantively closed at the Task 7 verification commit `a8c2364`).
...
Per the phase lifecycle state machine (`SKILL_ROUTING.md` line 17, verbatim from `BOOTSTRAP_PROMPT.md` §5 state 2): the next session — operating as the **state-2 → state-3 PLAN-writing session for sub-phase 05.2** — invokes **`superpowers:writing-plans`** scoped to the deliverables enumerated in `docs/envoy-rust/phases/05.2-http2-downstream/SPEC.md` §3 (D1–D10.2). Output: `docs/envoy-rust/phases/05.2-http2-downstream/PLAN.md` lands as a standalone pre-Task-1 commit per the 04.3 / 05.1 / 05.4 standardized cadence (precedent: 04.3's `c02eea7`, 05.1's `f23d08f`, 05.4's `252725b`). PLAN.md must enumerate the tasks (one per deliverable cluster), the per-task LoC estimate, the ADR landing points (ADR-0027 conditionally at Task 1 if `http` graduates from transitive-only to a workspace foundation; planner discretion per parent-05 SPEC §7 + 05.2 SPEC §1 layer 2), and the SPEC §6 signposts the planner has settled. The PLAN-write session must verify the SPLIT-GATE in `BOOTSTRAP_PROMPT.md` §6.1: if PLAN > ~25 tasks OR > ~1500 LoC estimated, split into sub-phases. 05.2's projected scope is the new `crates/envoy-http2/` workspace member (sole-dep-owner of `h2 = "0.4"`) + `CodecType::HTTP2` accept-flip + listener-side `Http2ProtocolOptions` schema + HCM-on-H2 dispatch + fixture 0009 + first conformance suite `tests/conformance/h2spec/` at the ≥95% pass gate + harness `Driver::Http2` + `drive_http2`; the planner should estimate at PLAN-write time whether this fits a single sub-phase or warrants further split (the parent-05 brainstorm explicitly bounded 05.2 to listener-side surfaces only, so the split-gate is unlikely to fire).
...
After the 05.2 phase-done commit (state-6), STATE.md advances active phase to `05.3-http2-upstream` lifecycle state 2 (PLAN.md does not exist for 05.3; SPEC was landed at parent-05 state-2 commit `f1804a7`). The **last** sub-phase commit (05.3's state-6) ALSO flips parent ROADMAP row `05` `in-progress` → `done` per the ROADMAP-schema invariant.

The DECISIONS.md ledger head is currently **ADR-0025** (landed at 05.4 Task 5 commit `01edie3`; landing-time order ADR-0023 → ADR-0024 → ADR-0026 → ADR-0025 per the append-only ledger discipline). 05.2 may land one new ADR conditionally at execution time (per parent-05 SPEC §7 + 05.2 SPEC §1 layer 2): a possible ADR-0027 for `http` typed-surface scoping (whether `http` graduates from transitive-only to a workspace foundation, or whether `envoy-http2` consumes `http` types only via re-exports through `h2`). Planner discretion at PLAN-write time. The DECISIONS.md ledger after 05.2 will read `... ADR-0025 (05.4 Task 5) | [ADR-0027 conditional] | ...` if landed.

Updated parent-05 sub-phase set (after this commit):

- **05.1 `fixture-hardening`** — DONE at commit `1d05cd0`. ADR-0023 landed at Task 1. Closed phase-02.1 REVIEW I3. **Partially closed phase-04.3 REVIEW C-1** (the schema + runtime + YAML preamble landed; fixture 0008 surfaced a different defect post-flip; substantive closure landed in 05.4).
- **05.4 `fixture-hardening-followup`** — DONE at this commit. ADR-0024 / ADR-0026 / ADR-0025 landed in landing-time order at Tasks 1 / 3 / 5. **Substantively closed phase-04.3 REVIEW C-1** at Task 7 verification commit `a8c2364` (CI run `25276504502` shows all 8 Docker-gated fixtures green simultaneously). The C-1 carryforward chain (originating at phase-02.2 ADR-0015 landing `435c6fa`, latent across phases 02.2 → 03.1 → 03.2 → 04.1 → 04.2 → 04.3) ends at this commit's predecessor `a88a986`.
- **05.2 `http2-downstream`** — PLANNED (active per STATE.md as of this commit; lifecycle state 2 — SPEC exists, PLAN.md does not). Next session: `superpowers:writing-plans` scoped to 05.2.
- **05.3 `http2-upstream`** — PLANNED; depends on 05.2.

Strict ordering: 05.1 (done) → 05.4 (done) → 05.2 (active, planned) → 05.3 (planned). 05.4's lexical id is after 05.3 in ROADMAP, but the execution ordering ran 05.1 → 05.4 → 05.2 → 05.3 per the disposition decision codified at the 05.1 state-6 commit; that disposition ends with 05.4's close-out at this commit, and the remaining ordering 05.2 → 05.3 follows lexical-id order naturally.

**Standing context for the 05.2 PLAN-writing session:**

- **The 8 Docker-gated baseline fixtures (0001-0008) are GREEN at this commit's predecessor `a8c2364`** per CI run `25276504502`. 05.2 inherits this baseline; fixture 0009 (the new H2-direct-response fixture) is the only new fixture in 05.2. The phase-done gate at 05.2 state-4 must keep all 9 fixtures green simultaneously.
- **PLAN.md cadence**: standalone pre-Task-1 commit per the 04.3 / 05.1 / 05.4 standardized posture (precedent: 04.3's `c02eea7`, 05.1's `f23d08f`, 05.4's `252725b`).
- **`h2spec` conformance suite** is the first conformance suite in the project. The ≥95% pass gate + `tests/conformance/h2spec/known-failures.txt` catalogue is the new pattern; the planner picks the h2spec invocation form at PLAN-write time per 05.2 SPEC §1 layer 6.
- **Cargo.lock cadence**: `h2 = "0.4"` is a NEW workspace dep landing at 05.2 Task 1. Per the 04.x / 05.1 / 05.4 inline-at-scaffold precedent (M5/M9 carryforward continues unchanged), Cargo.lock churn lands inline at the scaffold task.
- **`http` typed surface**: the `http` crate is currently transitive-only via `h2`. The 05.2 SPEC §1 layer 2 / 05.2 SPEC §6 signpost 21 frame an OPEN question — whether `http` graduates to a workspace foundation (would require ADR-0027) or stays transitive-only via the `h2`-re-export surface. Planner picks the posture at PLAN-write time.

Inputs the 05.2 PLAN-writing session should read, in order:
...
3. `docs/envoy-rust/ROADMAP.md` (rows 05.1 + 05.4 `done` after this commit; row 05 `in-progress`; rows 05.2 + 05.3 `planned`).
4. `docs/envoy-rust/DECISIONS.md` (all landed ADRs through `ADR-0025`; 05.2's conditional ADR-0027 starts at the next-sequential number).
5. `docs/envoy-rust/BEHAVIOR_CONTRACT.md` (Header allow-list section has 3 phase-04 rows; 05.2 engages Row 4 — HTTP/2 framing — for the first time per its SPEC §2; no edits anticipated).
6. `docs/envoy-rust/SKILL_ROUTING.md` (state machine).
7. **`docs/envoy-rust/phases/05.2-http2-downstream/SPEC.md`** — the design contract. §1 (5-layer goal + 6-bullet acceptance signal) → §3 (deliverables D5.2–D10.2) → §4 (non-goals) → §6 (signposts the planner must settle) → §7 (conditional ADR-0027 projection).
8. `docs/envoy-rust/phases/05-http2/SPEC.md` (parent-05 SPEC; 490 lines) — for cross-sub-phase architectural rules + non-goals 05.2 inherits.
9. `docs/envoy-rust/phases/05.4-fixture-hardening-followup/REVIEW.md` — §4 R-2 (`dns_lookup_family: V4_ONLY` recommended posture for any future fixture using STRICT_DNS with `host.docker.internal`; informational for 05.2's fixture 0009 which has no upstream cluster) + §4 R-4 (the `body_is_nonempty` predicate template applies to the H2 codec emission decision in 05.2).
10. `docs/envoy-rust/phases/05.1-fixture-hardening/SPEC.md` + `docs/envoy-rust/phases/05.4-fixture-hardening-followup/SPEC.md` — for the cluster-side STRICT_DNS posture inheritance (informational; fixture 0009 has `clusters: []`).
11. `docs/envoy-rust/phases/04.1-hcm-direct-response/SPEC.md` + `docs/envoy-rust/phases/04.3-router-upstream/SPEC.md` — for the HCM dispatch shape that 05.2's HCM-on-H2 reuses end-to-end.
12. `BOOTSTRAP_PROMPT.md` §5 state 2 (writing-plans skill routing) + §6.1 (split gate).

## Last commit

Phase 05.4 state-6 close-out commit (this commit): touches `docs/envoy-rust/ROADMAP.md` and `docs/envoy-rust/STATE.md` only. Flips ROADMAP row `05.4` `status: in-progress` → `done` (the row's other columns including the `summary` block are unchanged per the ROADMAP append-only invariant). Parent ROADMAP row `05` stays `status: in-progress` (will flip to `done` only after 05.2 + 05.3 close, per the schema invariant "parent flips to `done` only after all sub-phases are `done`"). ROADMAP rows `05.2` + `05.3` remain `status: planned`; 05.2 enters `in-progress` only at its own state-2 PLAN-writing session. STATE.md advances active phase from `05.4-fixture-hardening-followup` lifecycle state 5 (REVIEW.md Approved with M-track follow-ups) to `05.2-http2-downstream` lifecycle state 2 (SPEC.md exists at `f1804a7`; PLAN.md does not exist). Next-skill `superpowers:writing-plans` scoped to 05.2. Adds the "Phase-05.4 rollovers (from REVIEW.md §3–§5)" Notes section recording C-1 closed; phase-04.1 M-claim unblocked-but-not-closed; M-1 through M-5 awareness-only carryforwards. Updates the "Phase-05 ADR ledger (current)" Notes section to reflect ADR-0024 / ADR-0025 / ADR-0026 as LANDED (no longer projected); DECISIONS.md ledger head is now ADR-0025 (landing-time order: ADR-0023 → 0024 → 0026 → 0025).

No code changes at this commit. `ENVOY_TARGET.md` and `rust-toolchain.toml` untouched (D-3.7 / D-3.9). DECISIONS.md unchanged at this commit (ADR-0001 through ADR-0026 byte-identical; D-3.5 append-only).

The phase-04.3 REVIEW C-1 carryforward chain (Important, cross-phase systemic regression originating at phase-02.2 ADR-0015 landing `435c6fa`, latent across phases 02.2 → 03.1 → 03.2 → 04.1 → 04.2 → 04.3, partially closed at 05.1 state-6 `1d05cd0`) **ends at the immediate predecessor commit `a88a986`** with the substantive closure landing at Task 7 verification commit `a8c2364` (CI run `25276504502` shows all 8 Docker-gated fixtures green simultaneously). This is the SPEC §1 acceptance signal verbatim. See REVIEW.md §5 carryforward verdict + §6 verification gate observation for the cross-checked CI evidence.

Predecessor commits:

- `a88a986` — `phase 05.4: state 5 REVIEW.md Approved with M-track follow-ups` (state-5 close-out; verdict **Approved with M-track follow-ups**; zero Critical, zero Important on the 05.4 surface itself; 5 awareness-only Minor findings M-1 through M-5; the C-1 carryforward chain ends at this commit's CI run).
- `a8c2364` — `phase 05.4: state-4 phase-done gate verification (task 7)` (state-4 close-out; gate **GREEN** on all 8 Docker-gated fixtures simultaneously per CI run `25276504502`).
- `06c706d` — `phase 05.4: STRICT_DNS settle time 500ms→2000ms for host_gateway fixtures (task 6)` (Task 6).
- `01edie3` — `phase 05.4: envoy-http1 suppress synthetic content-length: 0 on empty-body + ADR-0025 (task 5)` (Task 5; ADR-0025 inline).
- `b72f1cb` — `phase 05.4: 3 echo-server helpers bind 0.0.0.0 (task 4)` (Task 4).
- `f1dbe12` — `phase 05.4: Listener.listener_filters parse-and-ignore + fixture 0006 tls_inspector + ADR-0026 (task 3)` (Task 3; ADR-0026 inline).
- `2bf4ba1` — `phase 05.4: 5-fixture coordinated YAML edit — dns_lookup_family: V4_ONLY (task 2)` (Task 2).
- `3d0af4a` — `phase 05.4: Cluster.dns_lookup_family + DnsLookupFamily enum + ADR-0024 (task 1)` (Task 1; ADR-0024 inline).
- `252725b` — `phase 05.4: state-2 PLAN.md (pre-Task-1 standalone per c02eea7 / f23d08f precedent)` (state-2 PLAN.md).
- `06b46a9` — `phase 05.4: state-2 brainstorm — SPEC.md (6 root-cause fixes; substantively closes phase-04.3 REVIEW C-1)` (state-2 brainstorm SPEC.md + ROADMAP row 05.4 added with `status: in-progress`).
- `1d05cd0` — `phase 05.1: ClusterType::StrictDns + 5-fixture coordinated edit [ADR-0023]` (phase 05.1 state-6 close-out).

## Last updated

2026-05-03 (phase 05.4 state-6 close-out commit; ROADMAP row 05.4 flipped `in-progress` → `done`; parent row 05 stays `in-progress` until 05.2 + 05.3 close; STATE advances active phase to `05.2-http2-downstream` lifecycle state 2; next-skill `superpowers:writing-plans` scoped to 05.2. Phase 05.4 closed in 7 substantive tasks across 9 commits between base `1d05cd0` and head `a8c2364` (plus this state-6 close-out commit and the state-5 REVIEW.md commit `a88a986`). REVIEW.md verdict **Approved with M-track follow-ups**; zero Critical, zero Important on the 05.4 surface itself; 5 awareness-only Minor findings; the phase-04.3 REVIEW C-1 carryforward chain ends at the immediate predecessor commit `a88a986`. ADR-0024 / ADR-0026 / ADR-0025 landed in landing-time order at Tasks 1 / 3 / 5 with no renumbering. The DECISIONS.md ledger head is now ADR-0025; ADR-0027 is conditionally projected at 05.2 Task 1 for `http` typed-surface scoping per parent-05 SPEC §7 + 05.2 SPEC §1 layer 2 (planner discretion).
...
### Earlier-phase carryforwards still open at 05.4 close

- **#13 — `x509-parser` deferred ADR.** Still deferred — 05.4 introduces no mTLS or peer-cert-attribution headers (no TLS work in 05.4 per its SPEC §2). Track forward to phase 05+ (post-05.4 sub-phases) or whichever phase first needs structured cert introspection.
- **#14 — `enable_half_close: true` flip-fixture.** Still deferred — 05.4 introduces no asymmetric-close semantics (no transport-level changes in 05.4 per its SPEC §2; ADR-0016 posture unchanged). Track forward to phase 05+ (post-05.4 sub-phases) or whichever phase first needs asymmetric-close semantics.
...
The DECISIONS.md ledger head is now **ADR-0025**. The 05.4 ADRs landed in DECISIONS.md in landing-time order, NOT numeric order: ADR-0023 (05.1 Task 1) → ADR-0024 (05.4 Task 1) → ADR-0026 (05.4 Task 3) → ADR-0025 (05.4 Task 5). The append-only ledger discipline is preserved (no retroactive renumbering).

Phase 05's projected ADRs after this commit (open for landing in 05.2 / 05.3):

- **ADR-0027** (CONDITIONAL — possibly at 05.2 Task 1 per parent-05 SPEC §7 + 05.2 SPEC §1 layer 2): `http` crate typed-surface scoping — whether `http` graduates to a workspace foundation (with its own deny.toml license-clearance entry as needed), or whether `envoy-http2` consumes `http` types only via re-exports through `h2`. Planner discretion at PLAN-write time.
