//! 29 Task 6 (ADR-0072) differential acceptance test for fixture
//! 0037-lb-maglev — MAGLEV consistent-hashing LB cross-proxy selection,
//! bilaterally asserted. A STATIC `lb_policy: MAGLEV` cluster `maglev_cluster`
...
//! selection (the §6.2-locked Maglev table reproduced end-to-end); SPREAD —
//! both backends are selected over the sweep on each side; STABILITY — a
//! repeated key hits the same backend on each proxy.
//!
...
#[tokio::test]
async fn lb_maglev_fixture() {
    let dir = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("..")
        .join("..")
        .join("tests/fixtures/0037-lb-maglev");
